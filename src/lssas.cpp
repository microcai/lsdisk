...
{
	boost::filesystem::directory_iterator by_bay_begin("/dev/disk/by-bay/"), by_bay_end;
	...
	for (auto dir_item : boost::make_iterator_range(by_bay_begin, by_bay_end))
	{
		...
		if (boost::regex_match(dir_item.path().filename().string(), matched, boost::regex("exp-[0-9]+-disk-[0-9]+")))
		{
